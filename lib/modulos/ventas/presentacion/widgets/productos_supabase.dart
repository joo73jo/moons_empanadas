...
class ProductosSupabase {
  static Future<List<String>> obtenerCategorias() async {
    try {
      ...
      for (final item in response) {
        ...
        final categoria = (mapa['categoria'] ?? '').toString().trim();
      ...
      final resultado = categorias.toList();

      resultado.sort((a, b) => a.toLowerCase().compareTo(b.toLowerCase()));

      return resultado;
    } on PostgrestException catch (e) {
      throw Exception('Error cargando categorías: ${e.message}');
    } catch (e) {
  ...
  static Future<List<ProductoVenta>> obtenerProductos() async {
    try {
      ...
      final componentesMap = <int, _ComponenteComboTemporal>{};
      ...
      for (final item in componentesResponse) {
        ...
        componentesMap[id] = _ComponenteComboTemporal(
          id: id,
          comboId: (mapa['combo_id'] as num).toInt(),
          nombreComponente: (mapa['nombre_componente'] ?? '').toString(),
          productoPredeterminadoId: (mapa['producto_predeterminado_id'] as num)
              .toInt(),
          cantidad: (mapa['cantidad'] as num?)?.toInt() ?? 1,
          permiteSustitucion: mapa['permite_sustitucion'] as bool? ?? true,
          obligatorio: mapa['obligatorio'] as bool? ?? true,
      ...
      if (componentesMap.isNotEmpty) {
        final opcionesResponse = await cliente
            ...
            ''')
            .eq('activo', true)
            .inFilter('componente_id', componentesMap.keys.toList())
            .order('componente_id')
            .order('orden');
        ...
        for (final item in opcionesResponse) {
          ...
          final componenteId = (mapa['componente_id'] as num).toInt();

          final temporal = componentesMap[componenteId];
          ...
          temporal.opciones.add(
            _OpcionComboTemporal(
              ...
              recargo: (mapa['recargo'] as num?)?.toDouble() ?? 0,
              activo: mapa['activo'] as bool? ?? true,
              orden: (mapa['orden'] as num?)?.toInt() ?? 0,
            ),
          );
      ...
      final componentesPorCombo = <int, List<ComponenteCombo>>{};

      for (final temporal in componentesMap.values) {
        final productoPredeterminado =
            productosMap[temporal.productoPredeterminadoId];
        ...
        for (final opcionTemporal in temporal.opciones) {
          final productoOpcion = productosMap[opcionTemporal.productoId];
          ...
          opciones.add(
            OpcionComponenteCombo(
              ...
              componenteId: opcionTemporal.componenteId,
        ...
        componentesPorCombo.putIfAbsent(temporal.comboId, () => []);

        componentesPorCombo[temporal.comboId]!.add(
          ComponenteCombo(
            id: temporal.id,
            comboId: temporal.comboId,
            nombreComponente: temporal.nombreComponente,
            productoPredeterminado: productoPredeterminado,
            cantidad: temporal.cantidad,
            permiteSustitucion: temporal.permiteSustitucion,
      ...
      for (final componentes in componentesPorCombo.values) {
      ...
      final resultado = productosMap.values.map((producto) {
        return producto.copyWith(
          componentesCombo: componentesPorCombo[producto.id] ?? const [],
        );
      }).toList();
  ...
  static Future<ProductoVenta> crearProducto(ProductoVenta producto) async {
    int? productoCreadoId;

    try {
      if (producto.seccion == SeccionVenta.combos &&
          producto.componentesCombo.isEmpty) {
        throw Exception('El combo debe tener al menos un componente.');
      }
      ...
      final creado = _mapearProductoBase(Map<String, dynamic>.from(respuesta));

      productoCreadoId = creado.id;
      ...
      if (producto.seccion == SeccionVenta.combos) {
        try {
          ...
          final componentesGuardados = await obtenerComponentesCombo(creado.id);

          if (componentesGuardados.isEmpty) {
            throw Exception(
              'El combo fue creado pero sus componentes no pudieron guardarse.',
            );
          }

          return creado.copyWith(componentesCombo: componentesGuardados);
        } catch (e) {
          try {
            await eliminarConfiguracionCombo(creado.id);
          } catch (_) {}
          ...
          throw Exception('No se pudo guardar la configuración del combo: $e');
        }
      }
      ...
      if (productoCreadoId != null) {
        try {
          await eliminarConfiguracionCombo(productoCreadoId);

          await SupabaseCliente.cliente
              .from('productos')
              .delete()
              .eq('id', productoCreadoId);
        } catch (_) {}
      }

      throw Exception('Error creando producto: ${e.message}');
    } catch (e) {
      if (productoCreadoId != null) {
        try {
          await eliminarConfiguracionCombo(productoCreadoId);
  ...
  static Future<ProductoVenta> actualizarProducto(
    ...
    try {
      ...
      if (producto.seccion == SeccionVenta.combos) {
        ...
        await eliminarConfiguracionCombo(producto.id);
      }

      return actualizado.copyWith(
        componentesCombo: producto.seccion == SeccionVenta.combos
            ? await obtenerComponentesCombo(producto.id)
            : const [],
      );
    } on PostgrestException catch (e) {
      throw Exception('Error actualizando producto: ${e.message}');
    } catch (e) {
  ...
  static Future<void> eliminarProducto(int id) async {
    try {
      ...
      throw Exception('Error eliminando producto: ${e.message}');
    } catch (e) {
      throw Exception('Error eliminando producto: $e');
    }
  }

  static Future<List<ComponenteCombo>> obtenerComponentesCombo(
    int comboId,
  ) async {
    try {
      final todosProductos = await SupabaseCliente.cliente
          .from('productos')
          .select()
          .eq('activo', true)
          .order('id');
      ...
      final componentesResponse = await SupabaseCliente.cliente
          .from('combo_componentes')
          .select('''
                id,
                combo_id,
      ...
      final componentesTemporales = <int, _ComponenteComboTemporal>{};
      ...
      for (final item in componentesResponse) {
        ...
        componentesTemporales[id] = _ComponenteComboTemporal(
          id: id,
          comboId: (mapa['combo_id'] as num).toInt(),
          nombreComponente: (mapa['nombre_componente'] ?? '').toString(),
          productoPredeterminadoId: (mapa['producto_predeterminado_id'] as num)
      ...
      final opcionesResponse = await SupabaseCliente.cliente
          .from('combo_componente_opciones')
          .select('''
                id,
                componente_id,
              ...
              ''')
          .eq('activo', true)
          .inFilter('componente_id', componentesTemporales.keys.toList())
          .order('componente_id')
          .order('orden');
      ...
      for (final item in opcionesResponse) {
        ...
        componentesTemporales[componenteId]?.opciones.add(
          _OpcionComboTemporal(
            id: (mapa['id'] as num).toInt(),
            componenteId: componenteId,
            productoId: (mapa['producto_id'] as num).toInt(),
      ...
      for (final temporal in componentesTemporales.values) {
        final predeterminado = productosMap[temporal.productoPredeterminadoId];
        ...
        for (final opcionTemporal in temporal.opciones) {
          final producto = productosMap[opcionTemporal.productoId];
        ...
        resultado.add(
          ComponenteCombo(
            ...
            comboId: temporal.comboId,
            nombreComponente: temporal.nombreComponente,
            productoPredeterminado: predeterminado,
            cantidad: temporal.cantidad,
            permiteSustitucion: temporal.permiteSustitucion,
            obligatorio: temporal.obligatorio,
            orden: temporal.orden,
      ...
      throw Exception('Error cargando componentes: ${e.message}');
    } catch (e) {
      throw Exception('Error cargando componentes: $e');
    }
  }
  ...
  static Future<void> guardarConfiguracionCombo({
    ...
    try {
      ...
      final idsExistentes = existentes
          .map<int>((item) => ((item as Map)['id'] as num).toInt())
          .toList();
      ...
      if (idsExistentes.isNotEmpty) {
        await cliente
            .from('combo_componente_opciones')
            .delete()
            .inFilter('componente_id', idsExistentes);
      }

      await cliente.from('combo_componentes').delete().eq('combo_id', comboId);

      if (componentes.isEmpty) {
        return;
      }

      final componentesOrdenados = List<ComponenteCombo>.from(componentes);

      componentesOrdenados.sort((a, b) => a.orden.compareTo(b.orden));

      for (int indice = 0; indice < componentesOrdenados.length; indice++) {
        final componente = componentesOrdenados[indice];

        if (componente.nombreComponente.trim().isEmpty) {
          throw Exception('Todos los componentes deben tener un nombre.');
        }
        ...
        if (componente.productoPredeterminado.id <= 0) {
          throw Exception(
            'Debes elegir el producto predeterminado de "${componente.nombreComponente}".',
          );
        }

        if (componente.productoPredeterminado.id == comboId) {
          throw Exception('El combo no puede contenerse a sí mismo.');
        }
        ...
        final componenteInsertado = await cliente
            ...
            .insert({
              'combo_id': comboId,
              'nombre_componente': componente.nombreComponente.trim(),
              'producto_predeterminado_id':
                  componente.productoPredeterminado.id,
              'cantidad': componente.cantidad,
              'permite_sustitucion': componente.permiteSustitucion,
        ...
        final componenteId = (componenteInsertado['id'] as num).toInt();

        if (!componente.permiteSustitucion || componente.opciones.isEmpty) {
          continue;
        }

        final opcionesUnicas = <int, OpcionComponenteCombo>{};
        ...
        for (final opcion in componente.opciones) {
          ...
          if (opcion.producto.id == comboId) {
            throw Exception('El combo no puede ser una opción de sí mismo.');
          }

          if (opcion.producto.id == componente.productoPredeterminado.id) {
            continue;
          }
          ...
          opcionesUnicas[opcion.producto.id] = opcion;
        }

        final opcionesOrdenadas = opcionesUnicas.values.toList();

        opcionesOrdenadas.sort((a, b) => a.orden.compareTo(b.orden));
        ...
        if (opcionesOrdenadas.isNotEmpty) {
          ...
          for (
            int opcionIndice = 0;
            opcionIndice < opcionesOrdenadas.length;
            opcionIndice++
          ) {
            final opcion = opcionesOrdenadas[opcionIndice];
          ...
          await cliente.from('combo_componente_opciones').insert(filas);
        }
      }
    } on PostgrestException catch (e) {
      throw Exception('Error guardando configuración del combo: ${e.message}');
    } catch (e) {
      throw Exception('Error guardando configuración del combo: $e');
    }
  }

  static Future<void> eliminarConfiguracionCombo(int comboId) async {
    final cliente = SupabaseCliente.cliente;
    ...
    try {
      ...
      if (ids.isNotEmpty) {
        await cliente
            .from('combo_componente_opciones')
            .delete()
            .inFilter('componente_id', ids);
      }

      await cliente.from('combo_componentes').delete().eq('combo_id', comboId);
    } on PostgrestException catch (e) {
      throw Exception('Error eliminando configuración del combo: ${e.message}');
    } catch (e) {
      throw Exception('Error eliminando configuración del combo: $e');
    }
  }

  static ProductoVenta _mapearProductoBase(Map<String, dynamic> mapa) {
    return ProductoVenta(
      id: (mapa['id'] as num).toInt(),
      nombre: (mapa['nombre'] ?? '').toString(),
      categoria: (mapa['categoria'] ?? '').toString(),
      precio: (mapa['precio'] as num?)?.toDouble() ?? 0,
