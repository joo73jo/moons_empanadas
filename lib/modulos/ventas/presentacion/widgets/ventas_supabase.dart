...
class VentasSupabase {
  ...
  static Future<int> guardarPedido({
    ...
    if (datosPedido.estadoCobro ==
            EstadoCobroVenta.cobradoRepartidor &&
        datosPedido.responsableDinero.trim().isEmpty) {
      throw Exception(
        'Debes indicar quién tiene pendiente el dinero.',
      );
    }

    if (datosPedido.requiereCobroInmediato &&
        resultadoCobro == null) {
      throw Exception(
    ...
    final cajaAbierta =
        await CajaSupabase.obtenerCajaAbierta();
    ...
    final totalRecargos =
        datosPedido.totalRecargos();

    final totalVenta = subtotal + totalRecargos;

    if (resultadoCobro != null &&
        (resultadoCobro.total - totalVenta).abs() >
            0.01) {
      throw Exception(
    ...
    final ventaInsertada = await cliente
        ...
        .insert({
          ...
          'banco': resultadoCobro == null ||
                  resultadoCobro.esPagoMixto
              ? null
              : resultadoCobro.banco,
          'datofono': resultadoCobro == null ||
                  resultadoCobro.esPagoMixto
              ? null
          ...
          'total': totalVenta,
          'valor_recibido':
              resultadoCobro?.valorRecibido,
          'cambio':
              resultadoCobro?.cambio ?? 0,
          'observacion':
              resultadoCobro != null &&
                      resultadoCobro.esPagoMixto
                  ? _crearObservacionPagoMixto(
                      resultadoCobro.pagos,
                    )
                  : null,
          'estado':
              estaPagado ? 'pagada' : 'pendiente',
          'estado_cobro':
              estadoCobroBaseDatos(
                datosPedido.estadoCobro,
              ),
          'responsable_dinero':
              datosPedido.responsableDinero
                      .trim()
                      .isEmpty
                  ? null
                  : datosPedido.responsableDinero
          ...
          'fecha_cobro':
              estaPagado
                  ...
                  : null,
          'enviado_preparacion':
              datosPedido.enviarPreparacion,
          'estado_preparacion':
              datosPedido.enviarPreparacion
                  ? 'pendiente'
                  : 'no_enviado',
          'fecha_envio_preparacion':
              datosPedido.enviarPreparacion
                  ? DateTime.now()
                      .toIso8601String()
                  : null,
    ...
    final ventaId =
        (ventaInsertada['id'] as num).toInt();
    ...
    for (final item in items) {
      final detalleInsertado = await cliente
          ...
          .insert({
            ...
            'precio_unitario':
                item.precioUnitarioFinal,
            'cantidad': item.cantidad,
            'subtotal': item.subtotal,
            'sabores': item.eleccionesCombo.isEmpty
                ? item.sabores
                : <String>[],
          })
          .select('id')
      ...
      if (item.eleccionesCombo.isNotEmpty) {
        ...
        await cliente
            .from(
              'detalle_venta_combo_elecciones',
            )
    ...
    if (datosPedido.recargos.isNotEmpty) {
      final recargosInsert =
          datosPedido.recargos.map((recargo) {
        return {
          'venta_id': ventaId,
          'recargo_configuracion_id':
              recargo.configuracionId,
      ...
      await cliente
          .from('venta_recargos')
          .insert(recargosInsert);
    }

    if (resultadoCobro != null) {
      final pagosInsert =
          resultadoCobro.pagos.map((pago) {
        return {
          'venta_id': ventaId,
          'metodo_pago':
              _mapearMetodoPago(
  ...
  static Future<void> cobrarVentaPendiente({
    ...
    final ventaResponse = await cliente
        .from('ventas')
        .select(
          'id, total, estado, estado_cobro, caja_id',
        )
        .eq('id', ventaId)
        .single();
    ...
    final estadoCobro =
        (venta['estado_cobro'] ?? '').toString();

    if (estadoCobro == 'pagado' ||
        estadoCobro == 'entregado') {
      throw Exception(
        'Este pedido ya fue cobrado.',
      );
    }

    if ((resultadoCobro.total - total).abs() >
        0.01) {
      throw Exception(
        'El total del cobro no coincide con el pedido.',
      );
    }
  ...
  static Future<List<RecargoConfiguracion>>
      obtenerRecargosConfiguracion({
    bool soloActivos = true,
  }) async {
    var consulta = SupabaseCliente.cliente
        .from('recargos_configuracion')
        .select(
          'id, nombre, porcentaje, activo, orden',
        );
    ...
    final response = await consulta.order(
      'orden',
      ascending: true,
    );

    return response
        .map<RecargoConfiguracion>((item) {
  ...
  static Future<RecargoConfiguracion>
      actualizarRecargoConfiguracion({
    ...
    if (nombre.trim().isEmpty) {
      throw Exception(
        'El nombre del recargo es obligatorio.',
      );
    }
  ...
  static Future<List<PedidoPreparacion>>
      obtenerPedidosPreparacion() async {
    final cliente = SupabaseCliente.cliente;
    ...
    final eleccionesPorDetalle =
        await _obtenerEleccionesComboPorDetalle(
      detalles
          .map<int>(
            (detalle) =>
                (detalle['id'] as num).toInt(),
          )
          .toList(),
    );
    ...
    final Map<
      int,
      List<DetallePedidoPreparacion>
    > detallesPorVenta = {};

    for (final detalle in detalles) {
      final ventaId =
          (detalle['venta_id'] as num).toInt();
      ...
      detallesPorVenta[ventaId]!.add(
        DetallePedidoPreparacion(
          ...
          sabores: sabores,
          eleccionesCombo:
              eleccionesPorDetalle[detalleId] ??
                  const [],
        ),
      );
    }
      ...
      return PedidoPreparacion(
        ...
        vendedorNombre:
            (usuario['nombre'] ?? '')
                .toString(),
        estadoPreparacion:
            (venta['estado_preparacion'] ??
                    'pendiente')
                .toString(),
        tipoPedido:
            tipoPedidoDesdeBaseDatos(
          venta['tipo_pedido'],
        ),
        barrio:
            (venta['barrio'] ?? '').toString(),
        estadoCobro:
            estadoCobroDesdeBaseDatos(
          venta['estado_cobro'],
        ),
        responsableDinero:
            (venta['responsable_dinero'] ?? '')
                .toString(),
        enviadoPreparacion:
            venta['enviado_preparacion']
                    as bool? ??
                true,
        subtotal:
            (venta['subtotal'] as num?)
                    ?.toDouble() ??
                0,
        totalRecargos:
            (venta['total_recargos'] as num?)
                    ?.toDouble() ??
                0,
        total:
            (venta['total'] as num?)
  ...
  static Future<List<PedidoPreparacion>>
      obtenerPedidosNoEnviados() async {
    final cliente = SupabaseCliente.cliente;
  ...
  static Future<Map<
    int,
    List<DetalleEleccionComboPreparacion>
  >> _obtenerEleccionesComboPorDetalle(
        ...
        DetalleEleccionComboPreparacion(
          ...
          cantidad:
              (mapa['cantidad'] as num?)
                  ...
                  1,
          recargoUnitario:
              (mapa['recargo_unitario'] as num?)
                      ?.toDouble() ??
                  0,
          recargoTotal:
              (mapa['recargo_total'] as num?)
                      ?.toDouble() ??
                  0,
          fueSustituido:
              mapa['fue_sustituido'] as bool? ??
                  false,
        ),
      );
    }
  ...
  static Future<void> marcarPedidoListo(
    int ventaId,
  ) async {
    await SupabaseCliente.cliente
        .from('ventas')
  ...
  static Future<void> enviarPedidoPreparacion(
    int ventaId,
  ) async {
  ...
  static Future<void> marcarDineroEntregado(
    int ventaId,
  ) async {
    await SupabaseCliente.cliente.rpc(
      'marcar_dinero_entregado',
      params: {
        'p_venta_id': ventaId,
      },
    );
  ...
  static Future<int> _obtenerUsuarioId(
    String usuarioLogin,
  ) async {
    final usuarioDb =
        await SupabaseCliente.cliente
  ...
  static String _crearObservacionPagoMixto(
    List<PagoCobro> pagos,
  ) {
  ...
  static Future<void>
      _validarStockAntesDeGuardar(
    List<ItemPedido> items,
  ) async {
    final cliente =
        SupabaseCliente.cliente;

    final descuentosPorId =
        <int, double>{};

    final descuentosPorNombre =
        <String, double>{};
    ...
    if (descuentosPorId.isNotEmpty) {
      final productos = await cliente
          .from('productos')
          .select(
            'id, nombre, stock_actual, controla_stock, activo',
          )
          .inFilter(
            'id',
            descuentosPorId.keys.toList(),
          );
      ...
      for (final item in productos) {
        ...
        final requerido =
            descuentosPorId[id] ?? 0;

        if (requerido >
            stockActual + 0.000001) {
          throw Exception(
    ...
    if (descuentosPorNombre.isNotEmpty) {
      ...
      for (final entry
          in descuentosPorNombre.entries) {
        final producto =
            porNombre[entry.key];
  ...
  static void _acumularDescuentosStock({
    ...
    for (final item in items) {
      if (item.eleccionesCombo.isNotEmpty) {
        for (final eleccion
            in item.eleccionesCombo) {
          final cantidad =
              eleccion.cantidad.toDouble() *
                  item.cantidad;
  ...
  static Future<void>
    ...
    final productos = await cliente
        .from('productos')
        .select(
          'id, nombre, stock_actual, controla_stock',
        )
        .inFilter(
          'id',
          descuentosPorId.keys.toList(),
    ...
    for (final entry
        in descuentosPorId.entries) {
      final producto =
          productosPorId[entry.key];
  ...
  static Future<void>
    ...
    double totalEfectivo =
        (caja['total_efectivo'] as num?)
                ?.toDouble() ??
            0;

    double totalTransferencia =
        (caja['total_transferencia'] as num?)
                ?.toDouble() ??
            0;

    double totalTarjeta =
        (caja['total_tarjeta'] as num?)
                ?.toDouble() ??
            0;

    double totalVentas =
        (caja['total_ventas'] as num?)
                ?.toDouble() ??
            0;
  ...
  static String _mapearMetodoPago(
